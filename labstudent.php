...
<head>
	<meta charset="UTF-8">
	<title>Лабораторные работы студентов</title>
...
<div id="cont">
     <center>
     <div id="labs">
     <h1>Лабораторные работы студентов</br> по предмету Прикладное Программирование</h1>
...
$sql = "SELECT * FROM polz"; 
$result = mysql_query($sql); 
while ($row = mysql_fetch_array($result)) { 

echo "</p><img src='jpg/doc.png'><a href= 'polz/". $row['name'] ."' download>" . $row['nazv'] . "</a>"; 
...
<?php
$resulter=mysql_query('SELECT * FROM `polz`'); 
while($rowler=mysql_fetch_array($resulter)){ 

if($_GET['login'] == $rowler['LoginUser']){ 
if($rowler['Priorety'] == "S"){ 

echo '<p><h3><font color="red">Перед тем как загружать файл убедитесь, что вы дали верное название файлу!</font></h3></p>
<p>Также вы можете загрузить свою, уже выполненную, лабораторную работу - 
<input id="sortpicture" type="file" name="sortpic" />'; 

echo '<button id="upload">Загрузить</button></p>'; 
